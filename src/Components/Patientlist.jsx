      ...
      if (!event) {
        if (patientdata.length > 0) {
          ...
          const appointmentdetails = {
            ...
            injury: patientdata[0].PersonalDetails.Reports.map((report, index) => (
              <div key={index}>
                {report && (
                  <div>
                    <span>{report}</span>
        ...
        const searchData = patientdata.find(pdata => pdata.user_id === event.user_id);
        if (searchData) {
          ...
          appointmentdetails.gender = searchData.PersonalDetails.PatientDetails.Gender;
          appointmentdetails.injury = searchData.PersonalDetails.Reports.map((report, index) => (
            <div key={index}>
              {report && (
                <div>
                  <span>{report}</span>
          ...
          <div className={`h-5/6 w-full `}>
            ...
            {screenWidth >= 500 && (
              ...
              >
                ...
                <div className={`h-1/2 w-full   flex flex-row`}>
                  <div className={`w-2/5 h-full  justify-start ${screenWidth<1200?"flex flex-row":"flex flex-col"}`}>
                    <Typography
                      variant="h6"
                      color="blue-gray"
                      className={`text-start ${screenWidth<1200?"w-2/5":"w-full"}`}
                    >
                      Diagnosis:
                    </Typography>
                    <Typography
                      variant="h6"
                      color="blue-gray"
                      className={`text-start ${screenWidth<1200?"w-3/5":"w-full"}`}
                    >
                     {tempdetails.injury}
                    </Typography>
                  </div>
                ...
                <div className={`w-full h-full flex flex-col justify-center gap-2`}>
                  ...
                  <div className={`w-full h-full flex justify-center`}>
                    <Typography
                      ...
                      className="text-start"
                    >
                      Diagnosis: {tempdetails.PersonalDetails.Reports.map((report, index) => (
                          <div key={index}>
                            {report && (
                              <div>
                                <span>{report}</span>
